...
const baseCursos = {
  primero:[
    ...
    "Historia del Pensamiento y de las Ideas","Estéticas I","Historia Medieval",
    "Literatura Medieval","Literatura del Mundo Antiguo","Museografía y Museología"
  ],
  segundo:[
  ...
  tercero:[
    ...
    "Fuentes de la Historia del Arte","Teoría y Metodología de la Historia del Arte",
    "Arte de las Vanguardias","Arte desde 1945 a la actualidad","Historia del Cine",
    "Patrimonio Histórico-Artístico y Gestión Cultural","OPT"           // slot nº 10 = OPT
  ],
  cuarto:[]
};
...
const optativasGeneral=[
  ...
  "La Ciudad en la Antigüedad","Paleografía y Diplomática","Picasso",
  "Pintura y Sociedad en la Edad Media","Velázquez","Prácticas Externas"
];

/* ========= Itinerarios ========= */
const itinerarios={
  "Arte Antiguo":{fix3:[],opt3:1,fix4:[
  ...
  "Arte Medieval":{fix3:[],opt3:1,fix4:[
  ...
  "Arte Contemporáneo":{fix3:[],opt3:1,fix4:[
      "Arquitectura y Modernidad","Arte Contemporáneo Latinoamericano","Goya","Picasso",
      "Arte Español Siglo XX","Historia de la Música II"
    ],opt4:3},
  "Museos":{fix3:["La Ciudad en la Antigüedad"],opt3:0,fix4:[
      "Artes Suntuarias y del Libro en la Edad Media","Arte, Catedral y Ciudad en la Edad Media Hispana","Al‑Ándalus: Ciudades, Mezquitas y Palacios",
      "Arquitectura y Ciudad en la Edad Moderna","Velázquez","Arquitectura y Modernidad",
      "Goya","Picasso","Historia de la Arqueología y del Coleccionismo de Antigüedades"
    ],opt4:0},
  "Arte Español":{fix3:[],opt3:1,fix4:[],opt4:9,special:[
      ...
      "Arte Español del Renacimiento","Velázquez","Arte Barroco Español",
      "Arte Hispanoamericano en la Edad Moderna","Arte Contemporáneo Latinoamericano",
      "Goya","Picasso","Arte Español Siglo XX","Prácticas Externas"
    ]}
};
...
const mkSelect=list=>{
  ...
  s.onchange=save; w.appendChild(s);
...
function build(){
  ...
  document.getElementById('nombre-itinerario').textContent=
     itinSel.value?`Itinerario: ${itinSel.value}`:'';
  ...
  const o3=itin?.opt3??1; for(let i=0;i<o3;i++)c3.appendChild(mkSelect(list));
  grid.appendChild(c3);
  ...
  const o4=itin?.opt4??9; for(let i=0;i<o4;i++)c4.appendChild(mkSelect(list));
  c4.appendChild(mkItem(\"Trabajo Fin de Grado (TFG)\")); grid.appendChild(c4);
...
function addCurso(nombre,arr){
  ...
  arr.forEach(a=>d.appendChild(mkItem(a))); grid.appendChild(d);
}
